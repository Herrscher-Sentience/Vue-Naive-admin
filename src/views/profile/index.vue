<template>
  <AppPage show-footer>
    ...
    <BasicModal ref="avatarModalRef" width="420px" title="更改头像" @ok="handleAvatarSave()">
      <n-input v-model:value="newAvatar" />
    </BasicModal>

    <BasicModal ref="pwdModalRef" title="修改密码" width="420px" @ok="handlePwdSave()">
      <n-form
        ref="pwdFormRef"
        :model="pwdForm"
        label-placement="left"
...
async function handleAvatarSave() {
  ...
  await user.updateProfile({ id: userStore.userId, avatar: newAvatar.value })
...
async function handleProfileSave() {
  ...
  await user.updateProfile(profileForm.value)

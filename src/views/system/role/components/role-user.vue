...
import { BasicCrud, BasicQueryItem } from '@/components/Table'
...
function handleBatchAdd(ids = userIds.value) {
  ...
  $dialog.confirm({
    ...
    async confirm() {
      await role.addRoleUsers(roleId, { userIds: ids })
      $table.value?.handleSearch()
    }
  })
}
function handleBatchRemove(ids = userIds.value) {
  ...
  $dialog.confirm({
    ...
    async confirm() {
      await role.removeRoleUsers(roleId, { userIds: ids })
      $table.value?.handleSearch()
    }
  })
}
